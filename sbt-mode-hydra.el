...
(defun sbt-hydra-command:run-test-only (project)
  `((sbt-hydra:run-test-only ,project) nil))
...
(defun sbt-hydra:run-test-only (project)
  (cond ((use-region-p)
...
(defun sbt-hydra:generate-hydra (current-project projects)
  (let* (
         (sbt-commands (append
                        ...
                        (list (sbt-hydra-command:run-test-only current-project))
